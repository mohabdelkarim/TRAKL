...
export async function requestAndShowConsent(): Promise<ConsentStatus> {
  ...
  // Expo Go: the native AdMob TurboModule isn't registered, and importing
  // the package throws synchronously during module evaluation (before the
  // try/catch can catch it). Skip entirely.
  if (isExpoGo) {
    return 'granted';
  }
